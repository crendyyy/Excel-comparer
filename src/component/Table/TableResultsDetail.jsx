...
const TableResultsDetail = () => {
  ...
  const { result, previousState } = location.state || {}; // Mengambil data dari state
  ...
  return (
    <div className="flex flex-col gap-8 p-10">
      <div className="flex gap-6">
        <Link
          to="/"
          state={previousState} // Mengirimkan kembali state yang sebelumnya
      ...
      <Table>
        <TableHead>
          <TableColumn className="gap-3 w-[200%]">
            <input type="checkbox" /> Nama Produk
          </TableColumn>
        ...
        <TableBody>
          {result.rows &&
            result.rows.map((row, index) => (
              <TableRow key={index}>
                <TableCell className="gap-3 w-[200%]">
                  <input type="checkbox" />
                  {row.nama_produk}
                </TableCell>
